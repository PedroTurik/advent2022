...
def is_impossible(max_cur_prune, ml, robots, materials, bp):
    return (materials[3]+((ml+1)*ml//2)+robots[3]*ml)<max_cur_prune


def checkBestOutcome(min_left, robots, materials, bp):
    global max_cur_prune

    if is_impossible(max_cur_prune, min_left, robots, materials, bp):
        return 0
    
    max_cur_prune = materials[3]
    if min_left == 0:
        return materials[3]
    max_val = 0 
...
counter = 1
for bp in tqdm(range(0,4)):
    max_cur_prune = 0
    counter *= checkBestOutcome(32, [1,0,0,0], [0,0,0,0], blueprints[bp])
